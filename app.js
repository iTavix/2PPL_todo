document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderDetail() {
    ...
    REFS.detail.backdrop.style.display = "flex"; REFS.detail.title.textContent = t.title; REFS.detail.notes.textContent = t.notes || ""; REFS.detail.parts.innerHTML = t.participants.map(p => `<div class="participant-chip">${p.name} ${p.done ? '✓' : ''}</div>`).join('');
  ...
  function openModal(todo) {
    ...
    if(todo) { REFS.modal.form.dataset.editId = todo.id; REFS.modal.inputTitle.value = todo.title; REFS.modal.inputDate.value = todo.date; } else { delete REFS.modal.form.dataset.editId; REFS.modal.form.reset(); REFS.modal.inputDate.value = toISO(state.selectedDate); }
  }
  ...
  REFS.landingForm.onsubmit = async (e) => {
    e.preventDefault();
    const { data, error } = await sb.auth.signInWithPassword({ email: REFS.landingEmail.value, password: REFS.landingPassword.value });
