document.addEventListener("DOMContentLoaded", () => {
  ...
  /* --- LANDSCAPE CLOCK & WEATHER LOGIC --- */
  function initLandscapeClock() {
    ...
    const updateTime = () => {
      ...
      // Formato Data (es: LUN 12 GEN)
      const dateStr = now.toLocaleDateString('it-IT', { weekday: 'short', day: 'numeric', month: 'short' });
      dateEl.textContent = dateStr.replace('.', '').toUpperCase();
    };

    // Aggiorna Meteo
    const fetchWeather = () => {
      if (!navigator.geolocation) {
         descEl.textContent = "Geolocalizzazione non supportata";
         return;
      }
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            // Usa Open-Meteo API (Gratuita, nessuna key necessaria)
            ...
            if (data.current_weather) {
              ...
              descEl.textContent = ""; // Pulisce il testo di caricamento
            }
          } catch (e) {
            console.error("Errore meteo", e);
            descEl.textContent = "Errore dati meteo";
          }
        },
        (err) => {
          console.warn(err);
          descEl.textContent = "Posizione necessaria per il meteo";
        }
      );
    };

    // Avvia orologio (ogni secondo)
    setInterval(updateTime, 1000);
    updateTime(); // Prima esecuzione immediata

    // Avvia Meteo (subito e poi ogni 15 min)
    fetchWeather(); 
